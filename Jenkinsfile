pipeline {
  ...
  environment {
    GHCR_IMAGE_BASE = 'ghcr.io/intranda/goobi-proxy-docker'
    DOCKERHUB_IMAGE_BASE = 'intranda/goobi-proxy-docker'
    NEXUS_IMAGE_BASE = 'nexus.intranda.com:4443/goobi-proxy-docker'
  }
      ...
      steps {
        withCredentials([
          ...
          sh '''
            ...
            docker buildx build \
              --platform linux/amd64,linux/arm/v5,linux/arm/v7,linux/arm64/v8,linux/386,linux/mips64le,linux/ppc64le,linux/s390x \
              $TAGS \
              --push .
          '''
        }
